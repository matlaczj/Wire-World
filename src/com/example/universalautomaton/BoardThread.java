...
public class BoardThread extends Thread {
	...
	public BoardThread(boolean isUpdating, byte chosenGame, int cols, Board board, int startingRow,
			int endingRow) {
		super();
		...
		for (int i = startingRow; i < endingRow; i++)
			for (int j = 1; j < cols - 1; j++) {
				...
				if (cellState == C.OFF && friendsWithStateON == 3)
					...
					board.getCell(i, j).setNextState(C.OFF); // przydatne zabezpieczenie przeciw komorkom WW ktore sa
																// teraz traktowane jak wylaczone
			}
	}
